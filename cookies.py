...
# AES decryption - install: pip install pycryptodome
...
def aes_cbc_decrypt_bytes(ciphertext, key, iv):
    cipher = AES.new(key, AES.MODE_CBC, iv)
    return cipher.decrypt(ciphertext)

def aes_gcm_decrypt_and_verify_bytes(ciphertext, key, tag, nonce):
...
try:
    ...
    secretstorage = None
    _SECRETSTORAGE_UNAVAILABLE_REASON = 'not available'
...
class MultilinePrinter:
    ...
    def print_at_line(self, *args): pass

class BaseError(Exception): pass
class DownloadError(BaseError): pass

expand_path = lambda p: os.path.expandvars(os.path.expanduser(p))
is_path_like = lambda p: isinstance(p, (str, bytes, os.PathLike))
sanitize_url = normalize_url = lambda u: u
str_or_none = lambda v, default=None: str(v) if v is not None else default
try_call = lambda f, *a, **k: (lambda: f(*a, **k))() if callable(f) else None
write_string = lambda s, out=sys.stderr: out.write(s)
error_to_str = str
...
from logger import Logger, _create_progress_bar
...
class CookieLoadError(BaseError):
    pass
...
def load_cookies(cookie_file, browser_specification):
    try:
        ...
        if browser_specification is not None:
            ...
            cookie_jars.append(
                extract_cookies_from_browser(browser_name, profile, Logger(), keyring=keyring, container=container))
...
def extract_cookies_from_browser(browser_name, profile=None, logger=Logger(), *, keyring=None, container=None):
